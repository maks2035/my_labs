...
def f_original(y, a, k):
   #a = 2(0.25)3
   #k = 0.25(0.25)1.25
   f = [ y[1] - (a * y[0] + k * y[1]) * y[0], np.exp(y[0]) - ( y[0] + a * y[1]) * y[0] ]
   return f
...
def explicit_euler_method(X, Y, Z, t, y, tau, T, a, k):
   X.append(t)
   Y.append(y[0])
   Z.append(y[1])
   f_ = f_original(y, a, k)
   ...
   Y.append(y[0])
   Z.append(y[1])
   if t < T: explicit_euler_method(X, Y, Z, t, y, search_step(e_first, f_original(y, a, k), T), T, a, k)
...
def f_euler_method(yk1, yk0, tau):##
   a = 2
   k = 0.25
   f = [ yk1[0] - yk0[0] - tau*(yk1[1] - (a * yk1[0] + k * yk1[1]) * yk1[0]),
         yk1[1] - yk0[1] - tau*(np.exp(yk1[0]) - ( yk1[0] + a * yk1[1]) * yk1[0] )]
   return f

def J_euler_method(X, tau):##
   a = 2
   k = 0.25
   j = [[1 - tau*(-2 * a * X[0] -k * X[1]), -tau*(-k * X[0])], 
        [-tau*(X[0]*np.exp(X[0]) -2 * X[0] - a * X[1]), 1 - tau*(-a*X[0])]]
   return j
...
def main(b):
   if b==1:
      ...
      explicit_euler_method(X1, Y11, Y21, t1, y1, search_step(e_first, f_original(y1, 2, 0.25), T_first), T_first, 2, 0.25)
   ...
   if b == 2:
      ...
      explicit_euler_method(X1, Y11, Y21, t1, y1, search_step(e_first, f_original(y1, a1, k1), T_first), T_first, a1, k1)
      ...
      explicit_euler_method(X2, Y12, Y22, t2, y2, search_step(e_first, f_original(y2, a3, k3), T_first), T_first, a2, k2)
      ...
      explicit_euler_method(X3, Y13, Y23, t3, y3, search_step(e_first, f_original(y3, a3, k3), T_first), T_first, a3, k3)
      ...
      plt.plot(X3, Y13, 'k--', label=''.join(['U1 с параметром a=',str(a3),", k=", str(k3)]))
      plt.plot(X3, Y23, 'k', label=''.join(['U2 с параметром a=',str(a3),", k=", str(k3)]))

      plt.rc('font',family='Times New Roman')
      ax.legend(bbox_to_anchor=(0.6, 1.15), fontsize = 14)
      plt.show()
