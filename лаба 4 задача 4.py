...
def f_original(y, l):
   #a = 1(0.25)2
   #k = 2(0.25)3
   f = [((2*l[0]+4*l[1])*y[0] + 2*(l[0]-l[1])*y[1] + 2*(l[0]-l[1])*y[2] + 4*l[0] + 2*l[1])/6,
        (2*(l[0]-l[1])*y[0] + (2*l[0]+l[1]+3*l[2])*y[1]+(2*l[0]+l[1]-3*l[2] + 4*l[0] - l[1] - 9*l[2] )*y[2])/6,
        (2*(l[0]-l[1])*y[0] + (2*l[0]+l[1]-3*l[2])*y[1] + (2*l[0]+l[1]+3*l[2] + 4*l[0] - l[1] + 9*l[2] )*y[2])/6]
   return f
...
def explicit_euler_method(X, Y, Z, U, t, y, T, l):
   X.append(t)
   Y.append(y[0])
   Z.append(y[1])
   U.append(y[2])
   while t < T:
      tau = search_step(e_first, f_original(y, l), T)
      f_ = f_original(y, l)
      for i in range(len(y)):
         y[i] = y[i] + tau*f_[i]
      t = t + tau
      ##
      if(round(t/T_first*100,1)%10 == 0):
         print("%.3f"%(t/T_first*100),"%")
      ##
      X.append(t)
      Y.append(y[0])
      Z.append(y[1])
...
def f_euler_method(yk1, yk0, tau, l):##
   f = [ yk1[0] - yk0[0] - tau*(((2*l[0]+4*l[1])*yk1[0] + 2*(l[0]-l[1])*yk1[1] + 2*(l[0]-l[1])*yk1[2] + 4*l[0] + 2*l[1])/6),
         yk1[1] - yk0[1] - tau*((2*(l[0]-l[1])*yk1[0] + (2*l[0]+l[1]+3*l[2])*yk1[1]+(2*l[0]+l[1]-3*l[2] + 4*l[0] - l[1] - 9*l[2])*yk1[2])/6),
         yk1[2] - yk0[2] - tau*((2*(l[0]-l[1])*yk1[0] + (2*l[0]+l[1]-3*l[2])*yk1[1] + (2*l[0]+l[1]+3*l[2] + 4*l[0] - l[1] + 9*l[2])*yk1[2])/6)]
   return f

def J_euler_method(X, tau, l):##
   j = [[1-tau*((2*l[0]+4*l[1])/6) , -tau*(2*(l[0]-l[1])/6), -tau*(2*(l[0]-l[1])/6)], 
        [-tau*(2*(l[0]-l[1])/6) , 1-tau*((2*l[0]+l[1]+3*l[2])/6) ,-tau*((2*l[0]+l[1]-3*l[2])/6) ],
        [-tau*(2*(l[0]-l[1])/6) , -tau*((2*l[0]+l[1]-3*l[2])/6), 1-tau*((2*l[0]+l[1]+3*l[2])/6)]]
   return j
...
def func_euler_method(X, tau):
   ...
   x1 = [0]*len(X)
   for i in range(500):
      a = матрицы.product_matrix_by_vector(матрицы.inverse_matrix(J_euler_method(x0, tau, l), len(X)), f_euler_method(x0, X, tau, l), len(X))
...
def main(b):
   if b == 1:
      ...
      explicit_euler_method(X1, Y11, Y21, Y31, t1, y1, T_first, l)
      ...
      #ax.legend(bbox_to_anchor=(0.5, 1.15), fontsize = 14)
      plt.show()
   if b == 2:
      X1 = []
      Y11 = []
